...
function sql_prep($query,&$db,$params=array(),$quickinsert=0){
	global $gsdbprofile;
	global $SQL_READONLY;
	...
	if ($verb=='insert'&&$quickinsert==0){//try to understand the query structure and rewrite the query
		...
		$myrs=sql_prep("describe table $table",$db);
		...
		if (isset($pkey)&&$pkey!=$firstfield){
			...
			foreach ($vlists as $vidx=>$vlist){
				$myrs=sql_prep('select generateUUIDv4() as uuid',$db);
	...
	if ($verb=='update'){
		...
		//sql_prep('set mutations_sync=2',$db);
	...
	if ($verb=='delete'){
		...
		//sql_prep('set mutations_sync=1',$db);
...
function sql_query($query,$db){
	...
	return sql_prep($query,$db,array());
}
...
function sql_patch($db,$table,$pkey,$recs,$wtable=null,$signfield='sign',$verfield='ver'){
/*
	$recs=array(
...
*/
	...
	//echo $query."\r\n"; print_r($params);
	sql_prep($query,$db,$params,1);


	//batch insert updates
	$query="insert into $usetable ($glist,$signfield,$verfield) values ".implode(',',$uqs);
	//echo $query."\r\n"; print_r($uparams);
	sql_prep($query,$db,$uparams,1);
	

}

/*
$db=sql_get_db('127.0.0.1:8123','mydb','default','mnstudio','db');
sql_patch($db,'products','productid',array(
	123=>null,
...
function sql_insert_id($db,$rs,$multi=0){
...
$db=sql_get_db('127.0.0.1:8123','gyrostart','default','mnstudio','db');
