...
<div class="section">
	...
	<textarea class="inplong" style="height:80px;"><?php echo base64_encode(serialize(mydbobj()));?></textarea>	
...
function tablecomp($my,$other){
	...
	foreach ($my as $k=>$v){
		if (!is_array($other[$k])) {$diffs[$k]=array('status'=>'dne_other');$same=0;}
	}
	
	foreach ($other as $k=>$v){
		if (!is_array($my[$k])) {$diffs[$k]=array('status'=>'dne_mine');$same=0;}
		else {
...
function sqlobjcomp($my,$other){
	...
	foreach ($my as $k=>$v){
		if (!is_array($other[$k])) $diffs[$k]=array('status'=>'dne_other');
	}
	
	foreach ($other as $k=>$v){
		if (!is_array($my[$k])) $diffs[$k]=array('status'=>'dne_mine');
		else {
