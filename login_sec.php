...
if ( (isset($_POST['password'])&&$_POST['password']) || (isset($_POST['gyroscope_login_'.$dkey])&&$_POST['gyroscope_login_'.$dkey]) ){	
	...
	if (!isset($fedbypass)&&$cfk!=$csrfkey&&$cfk!=$csrfkey2){
		...
		$password=md5(SQLDASH_DB_TOKEN.$_POST['password']);
		...
		if ($rateok){
			$query="select * from users where lower(login)=lower('".addslashes($login)."') and active=1 and (virtualuser=0 or virtualuser is null)";
			$rs=$sdb->query($query);
		} else {
		...
		if ($rateok&&$myrow=$rs->fetchArray(SQLITE3_ASSOC) ){
			...
			if ($useyubi&&($nopass||$yubimode==0||$yubimode==2)){
				...
				if ($c==0) $error_message='no security devices were found ';
					...
					$rs2=$sdb->query($query);
					if (!$myrow2=$rs2->fetchArray(SQLITE3_ASSOC)){
						$error_message="Cannot find a key in the registry.";	
					} else {
						$keyid=intval($myrow2['keyid']);
				...
				$passok=password_verify(SQLDASH_DB_TOKEN.$_POST['password'],$myrow['password']); //bcrypt uses its internal salt, $dbsalt here is just padding
				setcookie('fingername',NULL,time()-3600,null,null,$usehttps,true);
				if (!$passok) $passreset=0;
			}
		} else {
			password_hash(SQLDASH_DB_TOKEN.time(),PASSWORD_DEFAULT,array('cost'=>PASSWORD_COST));	
		}
		...
		if ($passok){
			...
			if ($usega&&$gakey!='') $gakey=decstr($gakey,GYROSCOPE_PROJECT.'gakey-'.$userid,1); //remote key
			...
			if (isset($_POST['passreset'])&&$_POST['passreset']){
				...
				if ($np!=$np2||$op==$np||trim($np)==''){
					...
					*/
						...
						$query="update users set password='$newpass', passreset=0, usega=0, useyubi=0 where userid=$userid";
						$sdb->query($query);
						$passreset=0;
					//}	
				}	  
			...
			if ($passreset){
				...
				if ($gaokay){	 
					...
					if (password_needs_rehash($myrow['password'],PASSWORD_DEFAULT,array('cost'=>PASSWORD_COST))){
						...
						$query="update users set password='$np' where userid=$userid";
						$sdb->query($query);
					}
					...
					$error_message=trim(implode('<br>',array($gaerror)),'<br>');
	...
	setcookie('connid',NULL,time()-3600,null,null,$usehttps,true);	
}
...
<div id="loginbox">
	...
	<div style="padding-top:5px;padding-bottom:15px;">
		<input class="lfinp" style="width:100%;" id="password" type="password" name="newpassword" onkeyup_="_checkpass(this);" onchange_="checkpass(this);">
...
?>
	...
	<script src="nano.js?v=4_10"></script>
	<script>
		function checkform(){
